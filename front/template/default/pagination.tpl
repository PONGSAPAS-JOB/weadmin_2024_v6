                    ...
                    <ul>
                        {if $pageStartShow > 1}
                            <li class="pagination-prev">
                                <a class="link"
                                    href="{$ul}{$pagination.method.page}{$pagination.method.parambefor}{$pagination.method.parameter}1">
                                    <span class="feather icon-chevrons-left"></span>
                                </a>
                            </li>
                        {/if}
                        {if $pagination.curent-1 > 0}
                            <li class="pagination-nav"><a class="link"
                                    href="{$ul}{$pagination.method.page}{$pagination.method.parambefor}{$pagination.method.parameter}{$pagination.curent-1}"><span
                                        class="link"><img src="{$template}/assets/img/icon/prev-sm.svg"
                                            alt="{$lang['pagination']['prev']}"></span></a></li>
                        {/if}

                        {for $current=$pageStartShow to $pageEndShow}
                            <li class="num-page {if $current == $pagination.curent}active{/if}"><a class="link"
                                    href="{$ul}{$pagination.method.page}{$pagination.method.parambefor}{$pagination.method.parameter}{$current}"
                                    title="{$txtLang['tools:page']} {$current}">{$current}</a></li>
                        {/for}
                        ...
                        {if $pagination.curent+1 < $pagination.totalpage}
                            <li class="pagination-nav"><a class="link"
                                    href="{$ul}{$pagination.method.page}{$pagination.method.parambefor}{$pagination.method.parameter}{$pagination.curent+1}"><span
                                        class="link"><img src="{$template}/assets/img/icon/next-sm.svg"
                                            alt="{$lang['pagination']['next']}"></span></a></li>
                        {/if}
                        {if $pagination.curent+2 < $pagination.totalpage}
                            <li class="pagination-nav">
                                <a class="link"
                                    href="{$ul}{$pagination.method.page}{$pagination.method.parambefor}{$pagination.method.parameter}{$pagination.totalpage}">
